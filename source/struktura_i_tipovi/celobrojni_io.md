...
где `format` садржи врсте улазник конверзија, а `lista_adresa` листу адреса
променљивих, записаних у облику `&identifikator` у које се уносе конвертоване
вредности учитаних података. Општи облик спецификатора конверзије за функцију
`scanf()` је следећи:
