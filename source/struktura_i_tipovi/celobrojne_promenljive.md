...
`int`. На пример, ако је задатак да напишеш програм у програмском језику C у
којем ћеш користити године ученика Тамаре и Владе, онда треба да декларишеш две
целобројне променљиве `t` и `v`. Програм можеш написати на следећи начин:
...
У оба случаја целобројне променљиве `t` и `v` декларисане су на исправан начин
...
се чува за знак, чиме се повећава опсег вредности које промељива може да
добије, па тако, неозначена целобројна променљива може да добије вредност од
$0$ до $4,294,967,295$, односно од $0$ до $2^{32}-1$.
...
бити већи од $31$. Према томе, променљива која треба да чува редни број
дана у месецу у програму можеш декларисати са `unsigned short`.
...
На пример, Влада на данашњи дан има 18, а Тамара 16 година. Напиши програм у
програмском језику C у којем ћеш иницијализовати променљиве `v` и `t` са
бројем година које Влада и Тамара имају на данашњи дан:
...
Према томе, претходни програм могао си написати на следећи начин:
...
ако су познате године Владе и Тамаре, као у претходном примеру, а нису познате
године Алисе и Боба, то можеш записати на следећи начин:
...
требаш дефинисати као константни податак, а не као променљиву.
...
Због тога ће се у следећем програму јавити грешка током компајлирања:
...
Такође, у току програма не можеш мењати вредност `c`, па ће се, због тога, у
следећем програму јавити грешка током компајлирања:
...
константе састоји из више речи, онда се речи раздвајају знаком `_`, на пример
`BRZINA_SVETLOSTI_U_VAKUUMU`. Тако би претходни задатак могао да решиш на
следећи начин:
...
односно симболом (који се назива и макро), замењује нека вредност. У примеру
изнад, целобројна вредност $299792458$ замењена је *симболом*
`BRZINA_SVETLOSTI_U_VAKUUMU`. Након дефиниције симболичке константе, њен
идентификатор можеш користити било где у програму уместо вредности. На пример:
...
int main(void)
{
    int c = BRZINA_SVETLOSTI_U_VAKUUMU;
}
```
...
препроцесорским директивана `#if defined()`, `#else` и `#endif`. На пример,
извршавањем следећег програма на стандардном излазу исписаће се
`Definisana je`:
...
а извршавањем следећег, исписаће се `Nije definisana`:
