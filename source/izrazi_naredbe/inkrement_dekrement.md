...
(енгл. *increment operator*) `++` којим се вредност операнда повећава за $1$ и
унарним оператором декрементирања (енгл. *decrement operator*) `--` којим се
...
се инкрементирати, односно декрементирати, пре него што се користи у изразу. У
постфиксном облику, операнд оператора `++`, односно, операнд оператора `--` ће
се прво користити у изразу, па тек након тога инкрементирати, односно
декрементирати.
...
је целобројна променљива `a = 4` иницијализована са оценом коју је Алиса
