...
Приметићеш да се за сваки реални тип података може користити чак осам симбола
конверзије f, F, g, G, e, E, a и A приликом формирања спецификатора конверзије.
...
словима на излазу исписаће све малим словима (нпр. inf, nan и e), док ће они са
великим словима исписати све великим словима (нпр. INF, NAN и E). Значи,
разлике су само естетске, а не и сусштинске, па на даље можеш користити нпр.
само спецификаторе са малим словима.

Симболи конверзије у спецификаторима за рад са хексадекадним реалним бројевима
a и A уведени су тек каснијим C99 стандардном и њих нећеш користити у овом
курсу.

Сада се број симбола конверзије за спецификаторе конверзије које требаш научити
...
датом редоследу можеш навести и следеће необавезне елементе:
...
минималну ширину поља наводи у листи аргумената функције која мора бити
...
У нотацији са децималном тачком, коришћењем додатне опције `.2` за
прецизност у спецификатору конверзије, на излазу би се исписали бројеви `4.85`,
`5.00` и `5.10`, па је у овом случају, свакако је једноставније прочитати
резултате у оптималној нотацији.
...
- `*`, карактер који означава да ће се улазни податак прочитати и
игнорисати,
- `širina`, целобројна вредност која задаје највећи број карактера улазног
податка,
- `dužina`, задаје се карактером `l` за `double`, или `L` за `long double`.

Реалне бројеве можеш уносити и у нотацији са децималном тачком и у научној
нотацији.

Напиши програм у програмском језику C који ће на стандардном излазу исписати
у оптимизованој нотацији дужину пута на релацији Београд-Вршац и релацији
Вршац-Београд у километрима, које се уносе са стандардног улаза. Приликом
тестирања програма унеси дужину пута Београд-Вршац као реалан број у нотацији
са децималном тачком `86.400000`, а дужима пута Вршац-Београд као реалан број у
